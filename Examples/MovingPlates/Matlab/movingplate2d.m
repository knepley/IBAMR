...
fid = fopen('../movingplate2d0.vertex','wt');
fprintf(fid,'%d\n', length(X_array));
...
%write the coordinates in the file
fid = fopen('../movingplate2d1.vertex','wt');
fprintf(fid,'%d\n', length(X_array));
...
%Y_array = Y_array - YCOM;
 %plot the plate
plot(X_array, Y_array, '.'); hold off;
%axis 'equal'

%write the coordinates in the file
fid = fopen('../movingplate2d2.vertex','wt');
fprintf(fid,'%d\n', length(X_array));
